...
from drift_retrain_model import StationaryModel, PeriodicalRetrain, DDMModel, PERMModel, STEPDModel
...
def experiment_driver(feature_list, label_list, out_file, n_round):
    ...
    models = [
        StationaryModel(MODEL_NAME, num_periods//2, DATASET),
        PeriodicalRetrain(MODEL_NAME, num_periods//2, DATASET),
        DDMModel(MODEL_NAME, num_periods//2, DATASET),
        PERMModel(MODEL_NAME, num_periods//2, DATASET),
        STEPDModel(MODEL_NAME, num_periods//2, DATASET)
    ]
    model_probas_list = [[] for _ in range(len(models))]

    # initial training features and labels (first sliding window)
    training_features = feature_list[: num_periods//2]
    training_labels = label_list[: num_periods//2]

    for idx, model in enumerate(models):
        np.random.seed(RANDOM_CONST+n_round*num_periods+num_periods//2-1)
        start_time = timeit.default_timer()
        model.initial_fit(training_features, training_labels)
        training_time = timeit.default_timer() - start_time

        start_time = timeit.default_timer()
        testing_probas = model.predict_proba(feature_list[num_periods//2])[:, 1]
        testing_time = timeit.default_timer() - start_time
        model_probas_list[idx].append(testing_probas)
        out_ls.append([model.get_name(), MODEL_NAME.upper(), n_round, num_periods//2 + 1, model.is_retrained(), training_time, testing_time] + obtain_metrics(label_list[num_periods//2], testing_probas))
    ...
    for i in range(num_periods//2, num_periods-1):
        ...
        for idx, model in enumerate(models):
            ...
            testing_time = timeit.default_timer() - start_time
            model_probas_list[idx].append(testing_probas)
            out_ls.append([model.get_name(), MODEL_NAME.upper(), n_round, i + 2, model.is_retrained(), training_time, testing_time] + obtain_metrics(testing_labels, testing_probas))
