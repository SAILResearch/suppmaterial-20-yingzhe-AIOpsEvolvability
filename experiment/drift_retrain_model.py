...
class PeriodicalRetrain(ConceptDetection):
    def __init__(self, model_name, window_size, dataset):
...
class StationaryModel(ConceptDetection):
    def __init__(self, model_name, window_size, dataset):
        super().__init__(model_name, window_size, dataset)
...
class DDMModel(ConceptDetection):
    def __init__(self, model_name, window_size, dataset):
        super().__init__(model_name, window_size, dataset)
        self.name = "DDM"
        self.p_min = 1.0
...
class PERMModel(ConceptDetection):
    def __init__(self, model_name, window_size, dataset):
        super().__init__(model_name, window_size, dataset)
        self.name = "PERM"
        self.P = 10
...
class STEPDModel(ConceptDetection):
    def __init__(self, model_name, window_size, dataset):
        super().__init__(model_name, window_size, dataset)
        self.name = "STEPD"

    def detect_concept(self):
        training_features = self.scaler.transform(self.feature_list[-2])
        testing_features = self.scaler.transform(self.feature_list[-1])
        training_err = np.count_nonzero(self.label_list[-2] != self.model.predict(training_features))
        testing_err = np.count_nonzero(self.label_list[-1] != self.model.predict(testing_features))
        _, pval = proportions_ztest([training_err, testing_err], [training_features.shape[0], testing_features.shape[0]])        
